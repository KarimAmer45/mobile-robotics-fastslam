...
class Particle(object):
    ...
    def prediction_step(self, odom):
        ...
        rot1 = random.gauss(odom.r1, s_rot1)
        trans = random.gauss(odom.t, s_trans)
        rot2 = random.gauss(odom.r2, s_rot2)

        x = self.pose[0, 0] + trans * cos(self.pose[2, 0] + rot1)
        y = self.pose[1, 0] + trans * sin(self.pose[2, 0] + rot1)
        theta = wrapToPi(self.pose[2, 0] + rot1 + rot2)
        ...
        for measurement in sensor_measurements:
            ...
            if not landmark.observed:
                ...
                landmark_mu = np.array(
                    [
                        self.pose[0, 0] + measurement.z_range * cos(bearing),
                        self.pose[1, 0] + measurement.z_range * sin(bearing),
                    ]
                )

                landmark.mu = landmark_mu

                # Compute landmark.sigma. Landmarks are assumed static, so there is no process noise!

                G_z = np.array(
                    [
                        [cos(bearing), -measurement.z_range * sin(bearing)],
                        [sin(bearing), measurement.z_range * cos(bearing)],
                    ]
                )
            ...
            # EKF Update on landmark
                ...
                K = landmark.sigma @ H.T @ np.linalg.inv(innovation_cov)

                # Compute compute the error between the z and expected_z (remember to normalize the angle)
                z_actual = np.array([measurement.z_range, measurement.z_bearing])
                innovation = np.array(
                    [
                        z_actual[0] - expected_z[0],
                        wrapToPi(z_actual[1] - expected_z[1]),
                    ]
                )
                ...
                norm = 1.0 / (2 * pi * sqrt(det))
                likelihood = norm * np.exp(
                    -0.5 * innovation.T @ np.linalg.inv(innovation_cov) @ innovation
                )
...
def low_variance_resampler(particles):
    ...
    offset = random.random() / N
    i = 0

    # Compared to EX06, particles is a list of objects!
    for m in range(N):
        position = offset + m / N
        while position > cumulative_weights[i] and i < N - 1:
            i += 1
